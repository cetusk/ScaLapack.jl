...
function eigvals!(d::Vector{T}, e::Vector{T}, blocksize = max(div(length(d), 100), 10)) where T<:Number
...
# function svdvals!(A::DArray{T,2,Matrix{T}}, blocksize::Integer = max(10, round(Integer, minimum(size(A))/10))) where T<:Number
